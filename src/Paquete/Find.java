...
public class Find extends JDialog implements ActionListener, KeyListener{
    ...
    public Find(Frame parent, boolean modal, int id_cli, long idadm) {
        super(parent, modal);
        try {
            Parent = parent;
            setTitle("Consultar Persona - ARTURO 1.0.5");
            setLayout(null);
    ...
    private void Textos() {
        LTitulo = new JLabel("CONSULTA DE PERSONAS");
        LTitulo.setFont(LTitulo.getFont().deriveFont(14f));
        LTitulo.setBounds(145, 2, 300, 20);

        LSub = new JLabel("Digite el ID de la persona del cual desea conocer la información registrada,");
    ...
    private void Botones() {
        ...
        BModificar.setBounds(60, 600, 100, 20);
        BModificar.addActionListener(this);
        add(BModificar);

        BEliminar = new JButton("Eliminar");
        BEliminar.setBounds(175, 600, 100, 20);
        BEliminar.addActionListener(this);
        add(BEliminar);

        BReiniciar = new JButton("Reiniciar");
        BReiniciar.setBounds(295, 600, 100, 20);
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if (ae.getSource() == BModificar) {
            ID = Integer.parseInt(TID.getText());
            setVisible(false);
            Modify AP = new Modify(Parent, true, ID, ID_ADM);
            dispose();
    ...
    public void keyPressed(KeyEvent ke) {
        ...
        if (key == KeyEvent.VK_F1) {
            JOptionPane.showMessageDialog(null, "• La ventana Consultar persona permite visualizar todos los datos registrados en el sistema \npara una persona.\n\n• Esta ventana no permite ningún tipo de modificación sobre los datos mostrados.\n\n• Para ver los datos de una persona debes digitar el número de su ID con el cual quedó registrado,\nluego pulsa el botón Consultar o la tecla Enter.\n\n• Si el ID coincide, en las casillas aparecerán los respectivos datos y se habilitarán los botones para\nmodificar o eliminar la persona.\n\n• Los botones Eliminar y Modificar solo se habilitarán cuando la consulta sea exitosa,\n es decir, que el ID consultado se encuentra registrado en el sistema.\n\n• El botón Reiniciar borra los datos que se encuentren en los campos y reinicia la consulta dejando\nel campo del ID en 0, listo para una nueva consulta.\n\nAl pulsar ACEPTAR esta ventana se cerrará.", "Ayuda Consultar Persona - ARTURO 1.0.5", JOptionPane.INFORMATION_MESSAGE);
        }
        if (ke.getSource() == TID) {
